...
class MobileapiController extends Controller
{
    ...
    public function capture_payment(Request $request)
    {
       // $arr_data['transaction_id']    = $request->input('transaction_id');
        ...
        $booking = \DB::table('nexa_booking')->where(['id'=>$token_id])->first();   
        ...
        {
            header("Location: https://nexa.marutiseva.com/failed.php");
            exit();
        }
        if($payment->status=="captured")
        {
            \DB::table('nexa_booking')->where(['id'=>$token_id])->update(['status'=>'Paid','booking_date'=>date('d/m/Y h:i:s A'),'transaction_id'=>$request->input('razorpay_payment_id')]);
...
<table width="900px" border="0" cellspacing="0" cellpadding="10" style="margin:0 auto; font-family:Arial, Helvetica, sans-serif; font-size:12px">
  ...
  <tr>
    <td><img src="https://www.marutiseva.com/assets/images/logo.png" /></td>
  ...
  <tr>
    ...
    <td align="center"><img src="https://www.marutiseva.com/admin/admin_css_js/css_and_js/maruti-suzuki-logo.png" width="180" /></td>
  </tr>
</table>
</td>
  </tr>
...
';
 mail($admin, $subject1, $message1, $headers1);   //admin mail
             ...
             header("Location: https://nexa.marutiseva.com/success.php");
            exit();
        }
        else
        {
...
public function offer_capture_payment(Request $request)
    {
       // $arr_data['transaction_id']    = $request->input('transaction_id');
        ...
        $token_id = base64_decode($request->input('token_id'));
        $offer_enquiries = \DB::table('nexa_offer_enquiries')->where(['id'=>$token_id])->first();       
        ...
        {
            header("Location: http://nexa.marutiseva.com/demo/failed.php");
            exit();
        }
        if($payment->status=="captured")
        {
            \DB::table('nexa_offer_enquiries')->where(['id'=>$token_id])->update(['status'=>'Paid','booking_date'=>date('d/m/Y h:i:s A'),'transaction_id'=>$request->input('razorpay_payment_id')]);
...
';
 mail($admin, $subject1, $message1, $headers1);   //admin mail
             ...
             header("Location: http://nexa.marutiseva.com/success.php");
            exit();
        }
        else
        {
            header("Location: http://nexa.marutiseva.com/failed.php");
            exit();
        }
    }
