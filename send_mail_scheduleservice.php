...
if ($city == 'Nashik' AND $area == 'Ambad')
{
    ...
    $admin = "seva.nnd.wm1@marutidealers.com";
        ...
        if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",$service_date)) {
          
        $new_date =  date('d-m-Y', strtotime($service_date));
        }else{
           
        $new_date =  $service_date;
        }
...
$message2.='<table border="0" cellpadding="2" cellspacing="2" style="width:500px;background-color:#D3D3D3">
	...
	<tbody><tr><td>';
		...
		$message2.='<tr>
			...
			<td style="width: 234px;"><span style="color:#000000"><span>'.$service_date.'</span></span></td>
		</tr>';
        $message2.='<tr>
        ...
        $message2.='<tr>
            ...
            <td style="width: 234px;"><span style="color:#000000"><span>'.$address.'</span></span></td>
        </tr>';
		$message2.='<tr>
...
$message2.='<span style="font-size:14px"><span style="font-family:arial,helvetica,sans-serif"><strong>Thank you</strong></span></span></body>
    ...
    mail($admin,$subject2,$message2,$headers2); // sends a copy of the message to the sender
     ...
     include('connection.php');
        ...
        $sql = "INSERT INTO `nexa_book_your_service`(`name`, `email`, `mobile_no`, `city`, `area`,`address`,`car_no`, `service_date`, `pickup`, `message`,`time`,`created_at`,`service`) VALUES ('$full_name','$to','$phone_no','$city','$area','$address','$reg_no','$service_date','$radio_stacked','$s_message','$time','$data_','$service')";
